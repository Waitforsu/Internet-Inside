...
# create two queues to manage the packet
tx_queue = multiprocessing.Queue()
rx_queue = multiprocessing.Queue()

# Read data from the TUN device and put it into the tx queue
def tun_reader(num):
    while True:
        # Read packets
        packet = tun.read(num)
...
# Write data from the rx queue to the TUN device
def tun_writer():
    while True:
        packet = rx_queue.get()
        tun.write(packet)
